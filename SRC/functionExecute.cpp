...
FunctionResult JavascriptArgEval(unsigned int index, char* buffer)
{
	...
	ReadCommandArg(arg,buffer,result,OUTPUT_NOTREALBUFFER|OUTPUT_EVALCODE|OUTPUT_UNTOUCHEDSTRING,MAX_ARG_BYTES);
	return result;
}
	...
	if (D->x.codeIndex && !(D->internalBits & (IS_PLAN_MACRO|IS_TABLE_MACRO))) // system function --  macroFlags are also on codeindex, but IS_TABLE_MACRO distinguishes  but PLAN also has a topicindex which is a codeindex
	{
		...
		while (ptr && *ptr != ')' && *ptr != ENDUNIT) // read arguments
		{
			...
			if ((trace & TRACE_OUTPUT || D->internalBits & MACRO_TRACE)  && !(D->internalBits & FN_NO_TRACE)&& CheckTopicTrace()) 
			{
				...
				Log(STDUSERLOG,(char*)" (%s), ",callArgumentList[callArgumentIndex]);
			}
...
static FunctionResult SetPositionCode(char* buffer)
{
	...
	if (*ptr == '_') // set match var position
	{
		...
		if (trace & TRACE_OUTPUT && CheckTopicTrace()) Log(STDUSERTABLOG, "_%d = %d...%d ",n,start,end);
...
{      
	unsigned int oldtrace = trace;
	if (trace & TRACE_NOTFULL)	trace = 0;
	FunctionResult result;
	*buffer = 0;
...
static void GenerateConceptList(int list, int set,char* filter,size_t len,int wordindex)
{
	while (list)
	{
...
static FunctionResult ConceptListCode(char* buffer)
{
	...
	for (int i = start; i <= end; ++i)
	{
		if (how & 1) GenerateConceptList(concepts[i], set,filter,len,i);
		if (how & 2) GenerateConceptList(topics[i], set,filter,len,i);
	}
	if (impliedSet == ALREADY_HANDLED && FACTSET_COUNT(set) == 0) return FAILRULE_BIT;
	impliedSet = ALREADY_HANDLED;
	currentFact = NULL;
	...
	case JSMN_OBJECT: {
		...
		*retMeaning = objectName;
		for (int i = 0; i < curr.size / 2; i++) { // each entry takes an id and a value
			...
			if (retToken == 0) return 0;
			CreateFact(objectName, keyMeaning, valueMeaning, jsonPermanent|flags|JSON_OBJECT_FACT); // only the last value of flags matters. 5 means object fact in subject
		}
		*flags = JSON_OBJECT_VALUE;
		break;
	}
	case JSMN_ARRAY: {
		...
		*retMeaning = arrayName;
		...
		for (int i = 0; i<curr.size; i++) {
			...
			if (retToken == 0) return 0;
			CreateFact(arrayName, index, arrayMeaning, jsonPermanent|flags|JSON_ARRAY_FACT); // flag6 means subject is arrayfact
		}
		*flags = JSON_ARRAY_VALUE; 
		break;
	}
...
static FunctionResult ParseJson(char* buffer, char* message, size_t size)
{
	*buffer = 0;
	...
	jsmn_init(&parser);
	jsmnerr_t tokenCount = jsmn_parse(&parser, message, size, NULL, 0);
	if (tokenCount > 0) 
	{
		// Now run it with the right number of tokens
		jsmntok_t *tokens = (jsmntok_t *)AllocateString(NULL,sizeof(jsmntok_t) * tokenCount,1,false);
		if (!tokens) return FAILRULE_BIT;
		jsmn_init(&parser);
		jsmn_parse(&parser, message, size, tokens, tokenCount);
...
static FunctionResult JSONParseFileCode(char* buffer)
{
	safeJsonParse = false;
	int index = JSONArgs();
	FILE* in = NULL;
